/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  IsOptional,
  MaxLength,
  ValidateNested,
} from "class-validator";
import { ProgressCreateNestedManyWithoutLearnersInput } from "./ProgressCreateNestedManyWithoutLearnersInput";
import { Type } from "class-transformer";
import { ReadingStatisticsCreateNestedManyWithoutLearnersInput } from "./ReadingStatisticsCreateNestedManyWithoutLearnersInput";

@InputType()
class LearnerCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  email?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => ProgressCreateNestedManyWithoutLearnersInput,
  })
  @ValidateNested()
  @Type(() => ProgressCreateNestedManyWithoutLearnersInput)
  @IsOptional()
  @Field(() => ProgressCreateNestedManyWithoutLearnersInput, {
    nullable: true,
  })
  progresses?: ProgressCreateNestedManyWithoutLearnersInput;

  @ApiProperty({
    required: false,
    type: () => ReadingStatisticsCreateNestedManyWithoutLearnersInput,
  })
  @ValidateNested()
  @Type(() => ReadingStatisticsCreateNestedManyWithoutLearnersInput)
  @IsOptional()
  @Field(() => ReadingStatisticsCreateNestedManyWithoutLearnersInput, {
    nullable: true,
  })
  readingStatisticsItems?: ReadingStatisticsCreateNestedManyWithoutLearnersInput;
}

export { LearnerCreateInput as LearnerCreateInput };
